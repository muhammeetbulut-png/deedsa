...
<body>
                ...
                <nav class="d-none d-lg-flex nav-links-wrap align-items-center">
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                            ...
                            <li><a class="dropdown-item" href="https://aistudio.google.com/app/apikey" target="_blank"><i class="fa-solid fa-circle-question text-info me-2"></i> API Nasıl Alınır?</a></li>
                        </ul>
                    </div>
                ...
                <div class="modal-body px-4 py-4">
                    <div class="mb-3">
                        ...
                        <div class="form-text small mt-2 text-muted">
                            <i class="fa-solid fa-shield-halved text-success me-1"></i> Girdiğiniz anahtar sunucuya gönderilmez, tarayıcınızda tutulur.
                        </div>
                    </div>
                </div>
    ...
    <main class="container py-4 view-section active" id="viewTerminal">
        ...
        <div id="loading" class="text-center my-5 d-none">
            ...
            <div class="text-dark fw-bold mt-3" id="loadingTitle">Yapay Zeka ile İsimler Türetiliyor...</div>
            <p class="text-secondary small mt-1" id="loadingSubtitle">API Anahtarınızla Google'a bağlanılıyor.</p>
        </div>
        ...
        <div id="resultsContainer" class="d-none">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <button class="btn btn-action" id="exportBtn" onclick="exportCSV()">
                    <i class="fa-solid fa-file-csv me-1 text-success"></i> <span>CSV İndir</span>
                </button>
            </div>
    ...
    <script>
        ...
        // =========================================================================
        // HATASIZ DİNAMİK MODEL ÇAĞIRICI (MIME KISITLAMASI YOK)
        // =========================================================================
        async function callGeminiDirectly(apiKey, promptText) {
            const modelsToTry = [
                "gemini-1.5-flash",
            ...
            // MIME type formatı kasıtlı olarak çıkarıldı (Eski modelleri bozmamak için)
            const payload = {
            ...
            for (const model of modelsToTry) {
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        errors.push(`[${model}] başarısız: ${errMessage}`);
                        
                        // Geçersiz anahtar ise boşuna diğer modelleri deneme, sistemi uyar
                        if (res.status === 400 && errMessage.toLowerCase().includes("api key not valid")) {
                            throw new Error("API Anahtarı geçersiz. Lütfen doğru kopyaladığınızdan emin olun.");
                        }
                    }
                } catch (e) {
                    if (e.message.includes("geçersiz")) throw e;
                    errors.push(`[${model}] ağ hatası: ${e.message}`);
                }
            }
            throw new Error("Google hiçbir modeli kabul etmedi. Hata: " + errors[0]);
        }
        // =========================================================================
        ...
        async function sendRobomMessage() {
            ...
            try {
                ...
                const replyText = await callGeminiDirectly(apiKey, systemPrompt);
        ...
        document.getElementById('searchForm').addEventListener('submit', async function(e) {
            ...
            document.getElementById('loadingTitle').innerText = "Yapay Zeka Modeline Bağlanılıyor...";
            document.getElementById('loadingSubtitle').innerText = "Tüm modeller otomatik taranıp en uygunu seçiliyor.";
    ...
    "domains": [
        {
            ...
            "score": 94
                ...
                const rawGeminiResponse = await callGeminiDirectly(apiKey, prompt);
                const generatedList = parseDomainsFromRaw(rawGeminiResponse);

                if (!generatedList || generatedList.length === 0) {
                    throw new Error("Yapay zeka listesi oluşturulamadı. Lütfen tekrar arama yapın.");
                }
