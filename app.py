...
# -------------------------------------------------------------
# 1. GÜVENLİK DUVARI (WAF) & GÜVENLİK BAŞLIKLARI
# -------------------------------------------------------------
...
<body>
                ...
                <div class="modal-body px-4 py-4">
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <input type="password" id="modalApiKeyInput" class="form-control search-input" placeholder="Gemini API Anahtarınızı Yapıştırın" style="font-family: monospace;">
                        ...
                        <div class="form-text small mt-2 text-muted">
                            <i class="fa-solid fa-shield-halved text-success me-1"></i> Gerçek Sıfır Bilgi Güvenliği: Anahtarınız DeedSa sunucusuna asla iletilmez. Çağrılar doğrudan tarayıcınızdan Google'a yapılır.
                        </div>
                    </div>
    ...
    <main class="container py-4 view-section active" id="viewTerminal">
        ...
        <div id="loading" class="text-center my-5 d-none">
            ...
            <div class="text-dark fw-bold mt-3" id="loadingTitle">E-Ticaret Pazar Verileri Taranıyor...</div>
            <p class="text-secondary small mt-1" id="loadingSubtitle">Yapay zeka ile 2 kelimelik marka kombinasyonları üretiliyor.</p>
        </div>
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-6">
                    ...
                    <div class="seo-faq-item">
                        ...
                        <p class="seo-faq-answer">Evet. DeedSa gerçek sıfır bilgi (zero-knowledge) prensibiyle çalışır. Girdiğiniz Google Gemini API anahtarı DeedSa sunucularına asla iletilmez; doğrudan kendi tarayıcınızdan Google API'ye bağlanır.</p>
                    </div>
                </div>
            </div>
        </section>
        ...
        <div class="row justify-content-center">
            <div class="col-lg-10">
                <div class="content-card">
                    ...
                    <p>DeedSa gerçek "Kendi Anahtarını Getir" (BYOK) mimarisiyle çalışır. Girdiğiniz Google Gemini API anahtarları yalnızca kendi tarayıcınızda tutulur ve doğrudan tarayıcınız üzerinden Google AI API'ye bağlanır. DeedSa sunucularına asla iletilmez, kaydedilmez ve loglanmaz.</p>
                </div>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        // DOĞRUDAN TARAYICIDAN GEMINI API İSTEĞİ (SIFIR SUNUCU HATASI & GÜNCEL MODELLER)
        async function callGeminiDirectly(apiKey, promptText, isJson = false) {
            // Google'ın 2026 itibarıyla en güncel aktif tuttuğu 3.5 serisi modelleri
            const models = [
                "gemini-3.5-flash",
            ...
            throw new Error(lastErr || "Gemini API bağlantısı kurulamadı.");
        }

        // ROBOM AI (TARAYICI ÜZERİNDEN DOĞRUDAN VE KESİNTİSİZ)
        ...
        async function processRobomQuery(userQuery) {
            ...
            try {
                const systemPrompt = `Sen Robom adında samimi ve yardımsever bir e-ticaret & alan adı asistanısın. DeedSa platformunda kullanıcılara rehberlik ediyorsun.
...
Robom:`;
                ...
                document.getElementById('robomThinking')?.remove();
                chat.innerHTML += `<div class="chat-msg robom text-danger">Hata: ${err.message}</div>`;
            }
            chat.scrollTop = chat.scrollHeight;
        }
        ...
        document.getElementById('searchForm').addEventListener('submit', async function(e) {
            ...
            document.getElementById('loadingTitle').innerText = "E-Ticaret Pazar Verileri Taranıyor...";
            document.getElementById('loadingSubtitle').innerText = "Yapay zeka ile 2 kelimelik marka kombinasyonları üretiliyor.";
    ...
    "domains": [
        {
            ...
            "score": 94
                ...
                if (!generatedList || generatedList.length === 0) {
                    throw new Error("Yapay zeka alan adı listesi oluşturamadı. Lütfen tekrar deneyin.");
                }

                document.getElementById('loadingTitle').innerText = "Canlı DNS Doğrulaması Yapılıyor...";
                document.getElementById('loadingSubtitle').innerText = `${generatedList.length} alan adı için küresel root DNS kayıtları taranıyor.`;

                // Üretilen domainleri sadece DNS kontrolü için sunucuya ilet (API Key sunucuya ASLA gitmez)
                ...
                if (data.status === 'success') {
                    ...
                    alert('DNS Doğrulama Hatası: ' + (data.detail || 'İşlem gerçekleştirilemedi.'));
                }
            } catch (err) {
