...
async def execute_gemini_request(http_client: httpx.AsyncClient, api_key: str, payload: dict) -> str:
    ...
    if not api_key:
        raise Exception("Lütfen geçerli bir Gemini API anahtarı giriniz.")

    # Render ortamında çalışan resmi model adları
    models_to_try = [
        "gemini-1.5-flash",
        "gemini-2.0-flash",
        "gemini-1.5-pro",
    ...
    for model_id in models_to_try:
        # Tekil yetkilendirme (Header üzerinden) - Çift anahtar çakışmasını önler
        url = f"https://generativelanguage.googleapis.com/v1beta/models/{model_id}:generateContent"
        try:
            resp = await http_client.post(url, headers=headers, json=payload, timeout=28.0)
    ...
    raise Exception(f"Gemini API yanıt üretemedi ({last_error})")
