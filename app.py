...
<head>
    ...
    <style>
        ...
        * { font-family: var(--font-main); box-sizing: border-box; scroll-behavior: smooth; }
        ...
        /* MOBİL UYUMLULUK */
        .mobile-menu-brand { font-size: 1.35rem; font-weight: 800; color: var(--primary); }
        .mobile-menu-section { font-size: 0.72rem; font-weight: 800; color: #94a3b8; letter-spacing: 0.08em; text-transform: uppercase; padding: 14px 12px 6px; }
        .mobile-nav-item { width: 100%; border: 0; background: transparent; color: #334155; font-weight: 700; text-align: left; padding: 11px 12px; border-radius: 10px; display: flex; align-items: center; gap: 10px; margin-bottom: 3px; }
        .mobile-nav-item:hover, .mobile-nav-item:active { background: var(--primary-light); color: var(--primary); }
        .mobile-nav-item i { width: 20px; text-align: center; }
        .mobile-menu-divider { border-top: 1px solid var(--border-color); margin: 10px 0; }
        @media (max-width: 991.98px) {
            .top-navbar { padding: 10px 16px; }
            .brand-logo { font-size: 1.5rem; margin-right: 0; }
            .hero-title { font-size: 2rem; padding: 0 12px; }
            .hero-subtitle-pro { padding: 0 16px; font-size: 0.98rem; }
            .terminal-card, .content-card { padding: 24px 18px; }
            .corporate-title { font-size: 2rem; }
            .robom-launcher { right: 16px; bottom: 16px; width: 56px; height: 56px; }
            .robom-window { right: 16px; bottom: 84px; width: calc(100vw - 32px); max-width: none; height: min(580px, calc(100vh - 105px)); }
            .footer { padding-left: 12px; padding-right: 12px; }
            .footer-link { margin: 0 5px; }
        }
    </style>
</head>
<body>
    ...
    <!-- MOBİL MENÜ -->
    <div class="offcanvas offcanvas-start" tabindex="-1" id="mobileOffcanvasMenu" aria-labelledby="mobileOffcanvasLabel" style="width: min(330px, 88vw);">
        <div class="offcanvas-header border-bottom">
            <div>
                <div class="mobile-menu-brand" id="mobileOffcanvasLabel">DeedSa</div>
                <div class="small text-secondary">Domain Intelligence Terminal</div>
            </div>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Kapat"></button>
        </div>
        <div class="offcanvas-body p-3">
            <div class="mobile-menu-section">Ana Menü</div>
            <button class="mobile-nav-item" data-bs-dismiss="offcanvas" onclick="switchView('terminal')"><i class="fa-solid fa-bolt text-primary"></i> Alan Adı Keşif Motoru</button>
            <button class="mobile-nav-item" data-bs-dismiss="offcanvas" onclick="switchView('whyToolsGuide')"><i class="fa-solid fa-book-open text-info"></i> Araçlar Rehberi</button>
            <button class="mobile-nav-item" data-bs-dismiss="offcanvas" onclick="switchView('whyDeedsa')"><i class="fa-solid fa-circle-info text-primary"></i> Neden DeedSa?</button>
            <button class="mobile-nav-item" data-bs-dismiss="offcanvas" onclick="switchView('whyRobom')"><i class="fa-solid fa-robot text-primary"></i> Robom AI Nedir?</button>
            <button class="mobile-nav-item" data-bs-dismiss="offcanvas" onclick="switchView('whyNamecheap')"><i class="fa-solid fa-cart-shopping text-success"></i> Neden Namecheap?</button>
            <button class="mobile-nav-item" data-bs-dismiss="offcanvas" onclick="switchView('whySedo')"><i class="fa-solid fa-globe text-primary"></i> Neden Sedo?</button>
            <button class="mobile-nav-item" data-bs-dismiss="offcanvas" onclick="switchView('howToEarn')"><i class="fa-solid fa-coins text-warning"></i> Nasıl Para Kazanılır?</button>

            <div class="mobile-menu-divider"></div>
            <div class="mobile-menu-section">Araçlar</div>
            <button class="mobile-nav-item" data-bs-dismiss="offcanvas" onclick="openToolModal('social')"><i class="fa-solid fa-share-nodes text-success"></i> Sosyal Medya Kullanıcı Adı Radarı</button>
            <button class="mobile-nav-item" data-bs-dismiss="offcanvas" onclick="openToolModal('logo')"><i class="fa-solid fa-palette text-info"></i> Logo & Mockup Üretici</button>
            <button class="mobile-nav-item" data-bs-dismiss="offcanvas" onclick="openToolModal('pitchbook')"><i class="fa-solid fa-file-lines text-warning"></i> E-Ticaret İş Planı</button>
            <button class="mobile-nav-item" data-bs-dismiss="offcanvas" onclick="openToolModal('outbound')"><i class="fa-solid fa-envelope-open-text text-danger"></i> Satış E-Postası</button>

            <div class="mobile-menu-divider"></div>
            <div class="mobile-menu-section">Gemini API</div>
            <button class="mobile-nav-item" data-bs-dismiss="offcanvas" data-bs-toggle="modal" data-bs-target="#apiKeyModal"><i class="fa-solid fa-key text-primary"></i> API Anahtarını Bağla / Yönet</button>
            <a class="mobile-nav-item text-decoration-none" href="https://aistudio.google.com/app/apikey" target="_blank"><i class="fa-solid fa-circle-question text-info"></i> Gemini API Nasıl Alınır?</a>

            <div class="mobile-menu-divider"></div>
            <div class="mobile-menu-section">Yasal & Destek</div>
            <button class="mobile-nav-item" data-bs-dismiss="offcanvas" onclick="switchView('terms')"><i class="fa-solid fa-scale-balanced"></i> Kullanım Koşulları</button>
            <button class="mobile-nav-item" data-bs-dismiss="offcanvas" onclick="switchView('affiliate')"><i class="fa-solid fa-handshake text-primary"></i> Affiliate Gelir</button>
            <button class="mobile-nav-item" data-bs-dismiss="offcanvas" onclick="switchView('privacy')"><i class="fa-solid fa-shield-halved text-success"></i> Gizlilik Politikası</button>
            <button class="mobile-nav-item" data-bs-dismiss="offcanvas" onclick="switchView('contact')"><i class="fa-solid fa-envelope text-primary"></i> İletişim</button>
        </div>
    </div>
    ...
    <script>
        ...
        function openToolModal(toolType) {
            ...
            if (toolType === 'social') {
                ...
                subEl.innerText = "Bir marka adı yazarak Instagram, X ve TikTok için canlı ön kontrol yapın.";
        ...
        async function runSocialCheck() {
            const input = document.getElementById('toolBrandInput');
            const resultsEl = document.getElementById('toolSocialResults');
            const val = input.value.trim().replace(/^@+/, '');
            if (!val) return;

            const button = input.parentElement.querySelector('button');
            if (button) { button.disabled = true; button.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span> Taranıyor...'; }
            resultsEl.innerHTML = '<div class="p-3 bg-light rounded-3 border text-center"><span class="spinner-border spinner-border-sm me-2"></span> Instagram, X ve TikTok kontrol ediliyor...</div>';

            try {
                const response = await fetch('/check-social-username', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ username: val })
                });
                const data = await response.json();
                if (!response.ok) throw new Error(data.detail || 'Kontrol başarısız.');

                const names = { instagram: 'Instagram', x: 'X / Twitter', tiktok: 'TikTok' };
                const badgeClass = { available: 'bg-success', taken: 'bg-danger', unknown: 'bg-warning text-dark' };
                const rows = data.results.map(item => `
                    <div class="d-flex justify-content-between align-items-center p-2 bg-white rounded border">
                        <span>${names[item.platform]}</span>
                        <span class="badge ${badgeClass[item.status] || 'bg-secondary'}">${item.label}</span>
                    </div>`).join('');

                resultsEl.innerHTML = `<div class="p-3 bg-light rounded-3 border">
                    <div class="fw-bold text-dark mb-2">"${data.username}" Hesap Taraması:</div>
                    <div class="d-flex flex-column gap-2">${rows}</div>
                    <div class="small text-secondary mt-3">Not: Platformların bot koruması veya giriş duvarı nedeniyle bazı sonuçlar "Belirsiz" görünebilir. Satın almadan/kullanmadan önce platform üzerinde son doğrulamayı yapın.</div>
                </div>`;
            } catch (error) {
                resultsEl.innerHTML = `<div class="alert alert-warning mb-0">${error.message || 'Sosyal medya kontrolü sırasında bir hata oluştu.'}</div>`;
            } finally {
                if (button) { button.disabled = false; button.innerHTML = 'Radarı Çalıştır'; }
            }
        }
...
class SocialUsernameRequest(BaseModel):
...
SOCIAL_RATE_HISTORY = defaultdict(list)
SOCIAL_RATE_WINDOW = 60
SOCIAL_MAX_REQUESTS = 20


def normalize_social_username(value: str) -> str:
    # Instagram/X/TikTok için ortak, güvenli kullanıcı adı karakterleri.
    value = value.strip().lstrip("@").lower()
    value = re.sub(r"[^a-z0-9._]", "", value)
    return value[:30]


async def check_social_platform(client: httpx.AsyncClient, platform: str, username: str) -> dict:
    urls = {
    ...
    try:
        ...
        # 404, bu üç platformda da hesabın bulunmadığına dair güçlü sinyaldir.
        if status == 404:
            return {"platform": platform, "status": "available", "label": "Müsait", "detail": "Profil bulunamadı."}

        # Platformun açıkça 'bulunamadı' dediği sayfalar.
        not_found_markers = {
        ...
        if any(marker in text for marker in not_found_markers[platform]):
            return {"platform": platform, "status": "available", "label": "Müsait", "detail": "Profil bulunamadı."}

        # Açık profil sinyalleri.
        taken_markers = {
            "instagram": ["instagram photos and videos", "followers", "following"],
            "x": ["joined", "followers", "posts"],
            "tiktok": ["followers", "following", "likes"],
        }
        hits = sum(1 for marker in taken_markers[platform] if marker in text)
        if status == 200 and hits >= 2:
            return {"platform": platform, "status": "taken", "label": "Kullanılıyor", "detail": "Profil bulundu."}

        # Login duvarı, rate-limit, bot koruması vb. durumlarda kesin sonuç verme.
        if status in (401, 403, 429) or status >= 500:
            return {"platform": platform, "status": "unknown", "label": "Kontrol edilemedi", "detail": f"Platform HTTP {status} döndürdü."}

        return {"platform": platform, "status": "unknown", "label": "Belirsiz", "detail": "Platform sonucu doğrulamaya izin vermedi."}
    except Exception as exc:
        return {"platform": platform, "status": "unknown", "label": "Kontrol edilemedi", "detail": "Bağlantı sırasında hata oluştu."}


@app.post("/check-social-username")
async def check_social_username(req: SocialUsernameRequest, request: Request):
    username = normalize_social_username(req.username)
    if len(username) < 2:
        raise HTTPException(status_code=400, detail="Geçerli bir kullanıcı adı girin.")

    client_ip = request.headers.get("x-forwarded-for") or request.client.host or "unknown"
    client_ip = client_ip.split(",")[0].strip()
    now = time.time()
    history = SOCIAL_RATE_HISTORY[client_ip]
    SOCIAL_RATE_HISTORY[client_ip] = [t for t in history if now - t < SOCIAL_RATE_WINDOW]
    if len(SOCIAL_RATE_HISTORY[client_ip]) >= SOCIAL_MAX_REQUESTS:
        raise HTTPException(status_code=429, detail="Sosyal medya kontrol kotasına ulaştınız. Lütfen biraz sonra tekrar deneyin.")
    SOCIAL_RATE_HISTORY[client_ip].append(now)

    async with httpx.AsyncClient(timeout=8.0) as client:
        results = await asyncio.gather(
            check_social_platform(client, "instagram", username),
            check_social_platform(client, "x", username),
            check_social_platform(client, "tiktok", username),
        )
