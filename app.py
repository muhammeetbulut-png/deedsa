...
# -------------------------------------------------------------
# 2. ARAYÜZ VE HTML İÇERİĞİ (TÜM ARAÇLAR, MODALLAR VE BLOGLAR DAHİL)
# -------------------------------------------------------------
HTML_CONTENT = """
<!DOCTYPE html>
...
async def check_single_domain(domain: str, explanation: str):
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        if res.status_code == 200:
            ...
            try:
                ...
                raise Exception("API yanıtı okunamadı.")
        else:
            try:
                err_msg = res.json().get("error", {}).get("message", "Bilinmeyen hata")
            except:
                err_msg = f"HTTP {res.status_code}"
            
            if res.status_code in [400, 403]:
                raise Exception(f"API Anahtarınız geçersiz veya Google tarafından engellenmiş. Lütfen Google AI Studio üzerinden yeni bir anahtar alınız. (Detay: {err_msg})")
...
async def analyze_category(req: CategoryRequest):
    prompt = f"""
    You are an elite global domain broker.
    TARGET NICHE: "{req.category}"
    Generate 25 brandable .COM domains (2 rhythmic English words each).
    Output ONLY valid JSON array with NO markdown ticks:
    [
      {{"domain": "pureglow.com", "explanation": "Bu e-ticaret sektörü için yüksek marka algısı sunan alan adı."}}
    ]
    """
    try:
        ...
        for item in domain_list:
            if isinstance(item, dict):
                ...
                if len(d) > 5 and d not in seen:
                    seen.add(d)
                    tasks.append(check_single_domain(d, item.get("explanation", "Müsait marka adı.")))
...
async def robom_chat(req: RobomChatRequest):
    ...
    try:
        ...
        clean_reply = reply.replace("**", "<strong>").replace("**", "</strong>").replace("\n", "<br>")
