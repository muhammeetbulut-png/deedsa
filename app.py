...
<head>
  ...
  <title>DeedSa | E-Ticaret Alan Adı Keşif ve Analiz Terminali</title>
...
<body class="bg-slate-50 text-slate-900 font-sans antialiased selection:bg-blue-600 selection:text-white pb-24">
  ...
  <!-- 1. ORTALANMIŞ AÇILIŞ ANİMASYONU (INTRO SCREEN) -->
  <div id="intro-screen" class="fixed inset-0 z-50 flex flex-col items-center justify-center bg-stars px-6 text-center">
    <div class="space-y-4 max-w-sm w-full mx-auto flex flex-col items-center justify-center">
      <h1 class="text-4xl md:text-6xl font-black tracking-tight text-white flex items-center justify-center gap-1">
        <span class="text-white">DEED</span><span class="text-sky-400">SA</span>
      </h1>
  ...
  <header class="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm">
    <div class="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
      ...
      <a href="/" class="flex items-center gap-2">
        ...
        <span class="hidden sm:inline-block text-[10px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">AI Terminal</span>
      </a>

      <!-- Masaüstü Linkler & API Butonu -->
      <nav class="hidden md:flex items-center gap-6 text-sm font-semibold text-slate-600">
        <a href="#araclar" class="hover:text-blue-600 transition">Araçlar Rehberi</a>
        <a href="#robom" class="hover:text-blue-600 transition">Robom AI</a>
        <a href="#arbitraj" class="hover:text-blue-600 transition">Arbitraj</a>
      ...
      <!-- Mobil Hamburger Buton (☰ 3 Çizgi) -->
      <button onclick="toggleMobileMenu()" class="md:hidden p-2 rounded-lg text-slate-700 hover:bg-slate-100 focus:outline-none" aria-label="Menü">
        <i class="fa-solid fa-bars text-xl"></i>
      </button>
    </div>

    <!-- Mobil Açılır Yan Menü (Drawer) -->
  ...
  <main class="max-w-4xl mx-auto px-4 pt-8 pb-12 space-y-10">
    ...
    <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-xl shadow-slate-200/50 space-y-4">
      <div>
        ...
        <input type="text" id="keywordFilter" placeholder="Kelime ile filtrele (örn: Retinol, Masaj, Deri...)" class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:outline-none focus:border-blue-600 focus:bg-white transition" />
      </div>
    ...
    <section id="sss" class="space-y-4 pt-4">
      ...
      <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
        ...
        <p class="text-xs text-slate-600 leading-relaxed">
          DeedSa, yapay zeka tarafından türetilen tüm domainleri anlık olarak ICANN akredite küresel DNS sunucularından canlı sorgular. Sadece kayda müsait olanlar önünüze getirilir.
        </p>
      </div>
      ...
      <div id="arbitraj" class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
        ...
        <p class="text-xs text-slate-600 leading-relaxed">
          DeedSa ile keşfettiğiniz yüksek ticari potansiyelli bir .com alan adını Namecheap üzerinden taban fiyata (10–12$) tescil edip, Sedo.com pazar yerinde global alıcılara (490$ - 2.500$) satışa sunarak arbitraj gerçekleştirebilirsiniz.
        </p>
      </div>
      ...
      <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
        ...
        <p class="text-xs text-slate-600 leading-relaxed">
          Evet. DeedSa sıfır bilgi (zero-knowledge) prensibiyle çalışır. Girdiğiniz Google Gemini API anahtarı sunuculara iletilmez, yalnızca kendi tarayıcınızın yerel hafızasında saklanır.
        </p>
      </div>
    </section>
    ...
    <footer class="pt-8 border-t border-slate-200 text-center space-y-4 text-xs font-medium text-slate-500">
      <div class="flex flex-wrap justify-center gap-x-4 gap-y-2">
        ...
        <span>•</span>
        <a href="#robom" class="hover:text-blue-600">Neden DeedSa?</a>
        <span>•</span>
        <a href="#robom" class="hover:text-blue-600">Robom AI Nedir?</a>
        <span>•</span>
        <a href="#namecheap" class="hover:text-blue-600">Neden Namecheap?</a>
        <span>•</span>
        <a href="#sedo" class="hover:text-blue-600">Neden Sedo?</a>
        <span>•</span>
  ...
  <!-- JAVASCRIPT KODLARI -->
  <script>
    // Açılış Animasyonunu 2.2 Saniye Sonra Gizle
    ...
    async function scanDomains() {
      ...
      try {
        ...
        if (data.domains && data.domains.length > 0) {
          ...
          alert('Uygun domain bulunamadı veya DNS kontrolünden geçemedi.');
        }
      } catch (err) {
        alert('Tarama sırasında bir hata oluştu: ' + err.message);
      } finally {
        btn.disabled = false;
...
# --- DNS / DOMAIN KONTROL YARDIMCISI ---
def is_domain_available(domain):
    """Canlı DNS sorgusu ile alan adının tescil edilip edilmediğini kontrol eder."""
    try:
        socket.setdefaulttimeout(1.5)
        socket.gethostbyname(domain)
        return False  # IP çözümlendiyse doludur
    except (socket.gaierror, socket.timeout):
        return True   # Çözümlenemediyse boştadır
...
def scan():
    ...
    niche = data.get("niche", "E-Ticaret")
    keyword = data.get("keyword", "")
    api_key = data.get("apiKey", "")

    # Örnek ve Gemini tabanlı domain adayı listesi
    base_words = ["glow", "pulse", "store", "hub", "fit", "pure", "core", "shop", "wave", "prime"]
    prefix = keyword.lower().strip() if keyword else "deed"
    
    generated_domains = [f"{prefix}{w}.com" for w in base_words]
    
    # DNS doğrulaması yaparak sadece boştakileri listele
    available = []
    ...
    return jsonify({"domains": available})

if __name__ == "__main__":
    port = int(os.environ.get("PORT", 10000))
    app.run(host="0.0.0.0", port=port)# -------------------------------------------------------------
# 2. ARAYÜZ VE HTML İÇERİĞİ (TÜM MEVCUT + YENİ EKSİKSİZ KATEGORİLER)
# -------------------------------------------------------------
HTML_CONTENT = """
<!DOCTYPE html>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-7">
                        ...
                        <select id="category" class="form-select form-select-pro">
                            <!-- MEVCUT ÖZEL VE DERİN NİŞLER (KORUNDU) -->
                            ...
                            <!-- TÜM YENİ VE TAMAMLAYICI KATEGORİLER -->
                            ...
                            <option value="Duş Perdeleri">Duş Perdeleri</option>
                        ...
                        domainList.innerHTML += `
                            <div class="investment-card">
                                ...
                                <div class="sedo-banner">
                                    ...
                                    <a href="https://sedo.com" target="_blank" class="btn-sedo-blue"><span>Sedo'da Satışa Çıkar</span><i class="fa-solid fa-chevron-right"></i></a>
                                </div>
                            </div>
                        `;
                    });
    ...
    for model_id in target_models:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                last_err = f"{model_id} -> HTTP {resp.status_code}"
        except Exception as e:
            last_err = str(e)
            continue

    raise Exception(f"Gemini API yanıt üretemedi. Anahtarınızı kontrol edin ({last_err})")
